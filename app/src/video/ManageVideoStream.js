...
import VideoAudioRoom from './VideoAudioRoom';

const ManageVideoStream = ({username, setError}) => {
  ...
  const startStream = async (event) => {
    ...
    try {
      const response = await fetch('http://localhost:5000/twilio-live/video/start', {
        method: 'POST',
        headers: {
  ...
  const endStream = async (event) => {
    ...
    try {
      const response = await fetch('http://localhost:5000/twilio-live/end', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify(streamDetails)
      });
        ...
        <Column span={8}>
          ...
          {isLive &&
            <Card>
              ...
              <VideoAudioRoom streamDetails={streamDetails} room={room} />
...
export default ManageVideoStream;